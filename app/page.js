...
export default function Home() {
  ...
  const removeFromCart = (productId) => {
    ...
    // Menambahkan kembali stok setelah item dihapus dari keranjang
    setProducts((prevProducts) =>
      prevProducts.map((item) =>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-200 p-8">
      <h1 className="text-4xl font-bold text-center mb-10 text-purple-800">Katalog Produk</h1>

      {/* Menampilkan Produk */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition duration-300 border border-purple-200">
            <img
              src={product.image}
              alt={product.title}
              className="w-full h-64 object-cover rounded-lg mb-4"
            />
            <h2 className="text-xl font-semibold text-purple-700">{product.title}</h2>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`flex items-center justify-center bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 transition duration-300 ${product.stock === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
              ...
              cart.map((item) => (
                <div key={item.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() =>
                        handleQuantityChange(item.id, Math.min(item.quantity + 1, item.stock))
                      }
